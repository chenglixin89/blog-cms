...
public interface UserFavoriteMapper {
    ...
    @Insert("""
        INSERT IGNORE INTO blog_user_favorite (user_id, article_id)
        VALUES (#{userId}, #{articleId})
        """)
    int insert(@Param("userId") Long userId, @Param("articleId") Long articleId);

    @Delete("DELETE FROM blog_user_favorite WHERE user_id = #{userId} AND article_id = #{articleId}")
    int delete(@Param("userId") Long userId, @Param("articleId") Long articleId);

    @Select("SELECT COUNT(*) FROM blog_user_favorite WHERE user_id = #{userId} AND article_id = #{articleId}")
    int countByUserAndArticle(@Param("userId") Long userId, @Param("articleId") Long articleId);

    @Select("SELECT COUNT(*) FROM blog_user_favorite WHERE user_id = #{userId}")
    ...
    @Select("""
        ...
        FROM blog_user_favorite f
        JOIN blog_article a ON a.id = f.article_id
        ...
        """)
    List<Article> selectPublishedFavoritesByUserId(@Param("userId") Long userId);

    @Select("SELECT article_id FROM blog_user_favorite WHERE user_id = #{userId}")
